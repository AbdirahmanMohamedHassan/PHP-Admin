...
$sql = "SELECT * FROM tb_shortcourse,tb_professor where tb_shortcourse.course_professor = tb_professor.professor_id and tb_shortcourse.course_code='$_GET[course_code]'";
...
?>
                                        <div class="form-group row">
                                            ...
                                            <div class="form-group row">
                                                ...
                                                <div class="col-md-5">
                                                    <textarea id="summernote" style="tabsize:2;height:100" name="course_description" placeholder="course details" class="form-control-textarea" rows="5" ><?php echo$row["course_description"] ?></textarea>
                                                </div>
                                            </div>
											  <div class="form-group row">
				...
				function pro() {
					include ("db.php");
    $sql = "SELECT * FROM tb_shortcourse,tb_professor where tb_shortcourse.course_code='$_GET[course_code]'";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
	while($row = $result->fetch_assoc()) {
		?>
...
if(isset($_POST['submit'])){
	...
	if( empty($_POST['course_name'])  || empty($_POST['course_description']) || empty($_POST['course_professor']) || empty($_POST['course_duration'])
        ...
        <strong>Woah!</strong> Fill out all the information please.
		 ...
		 header("location:edit_short_course.php?course_code=$_GET[course_code]");
	}else{
...
// active is 0 by DEFAULT (no need to include it here)
    ...
    $sql="UPDATE tb_shortcourse SET course_name='$course_name' , course_description='$course_description' , course_professor='$course_professor'
