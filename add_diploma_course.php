...
if(isset($_POST['submit'])){
	...
	if( empty($_POST['course_name']) || empty($_POST['course_code']) || empty($_POST['course_description']) || empty($_POST['course_professor']) || empty($_POST['course_duration'])
        ...
        <strong>Woah!</strong> Fill out all the information please.
		 ...
		 header("location:add_diploma_course.php?diploma=$diploma_code");
	}else{
...
// active is 0 by DEFAULT (no need to include it here)
    $sql = "INSERT INTO tb_diplomacourse (course_name,course_code,course_description,course_professor,course_duration,course_price,course_level,course_image,diploma_code,course_date) " 
            . "VALUES ('$course_name','$course_code','$course_description','$course_professor','$course_duration','$course_price','$course_level','$filepath','$diploma_code','$date')";
echo $sql;
    // Add user to the database
    if ( $mysqli->query($sql) ){
		$_SESSION['adminmessage']= " <div class='alert alert-success'>
        <strong>Well done!</strong> You successfully added a new diploma course, it will be appear on the website.
                    </div>";
		 header("location:diploma_course.php?diploma=$diploma_code");
	}
else {
   $_SESSION['adminmessage']= " <div class='alert alert-danger'>
        <strong>Warning!</strong> You weren't successful in adding a new diploma course.
                    </div>";
header("location:diploma_course.php?diploma=$diploma_code");
    }
}}}
